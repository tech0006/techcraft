...
public class TCbenchContainer extends AbstractContainerMenu {

    public TCbenchBlockEntity blockEntity;
    private Player playerEntity;
    private IItemHandler playerInventory;

    public TCbenchContainer(int windowId, Level world, BlockPos pos, Inventory playerInventory, Player player) {
        super(TCContainerTypes.TC_BENCH.get(), windowId);
        blockEntity = (TCbenchBlockEntity) world.getBlockEntity(pos);
        ...
        if (blockEntity != null) {
            blockEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h -> {
                for (int i = 0; i < 6; i++) {
                    for (int j = 0; j < 6; j++) {
                        addSlot(new SlotItemHandler(h, i * 6 + j, 17 + 18 * j, 17 + 18 * i));
                    }
                }
                addSlot(new SlotItemHandler(h, 36, 134, 62) {
                    @Override
    ...
    public boolean stillValid(Player playerIn) {
        return stillValid(ContainerLevelAccess.create(blockEntity.getLevel(), blockEntity.getBlockPos()), playerEntity, TCBlocks.TC_BENCH.get());
    }
