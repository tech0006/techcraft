...
public class CopperSafe extends FacedBlock {
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return ModTileEntityTypes.COPPER_SAFE.get().create();
    }
    ...
    public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult result) {
        if (!worldIn.isClientSide) {
            ...
            if (tile instanceof CopperSafeTileEntity) {
                NetworkHooks.openGui((ServerPlayerEntity) player, (CopperSafeTileEntity) tile, pos);
    ...
    public void onRemove(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (te instanceof CopperSafeTileEntity) {
                InventoryHelper.dropContents(worldIn, pos, ((CopperSafeTileEntity) te).getItems());
            }
        }
    }
