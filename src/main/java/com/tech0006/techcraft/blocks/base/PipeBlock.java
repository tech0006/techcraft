...
public class PipeBlock extends BlockTransmitter {
    ...
    public VoxelShape getShape(BlockState state, IBlockReader world, BlockPos pos, ISelectionContext context) {
            ...
            if (tile != null) {
                if (tile.getLevel() != null) {
                    tile.getLevel().setBlockAndUpdate(pos, this.stateDefinition.any().setValue(UP, u).setValue(DOWN, d)
                            .setValue(NORTH, n).setValue(EAST, e).setValue(SOUTH, s).setValue(WEST, w));
                }
            }
        ...
        if (u) { list.add(Block.box(7, 9, 7, 9, 16, 9)); }
        if (d) { list.add(Block.box(7, 0, 7, 9, 7, 9)); }
        if (n) { list.add(Block.box(7, 7, 0, 9, 9, 7)); }
        if (s) { list.add(Block.box(7, 7, 9, 9, 9, 16)); }
        if (w) { list.add(Block.box(0, 7, 7, 7, 9, 9)); }
        if (e) { list.add(Block.box(9, 7, 7, 16, 9, 9)); }
