...
import com.tech0006.techcraft.blocks.TCforge;
...
public class TCforgeTileEntity extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
    ...
    public TCforgeTileEntity() {
        this(ModTileEntityTypes.TC_FORGE.get());
    }

    @Override
    public Container createMenu(final int windowID, final PlayerInventory playerInv, final PlayerEntity playerIn) {
        return new TCforgeContainer(windowID, playerInv, this);
    }
        ...
        if (this.level != null && !this.level.isClientSide) {
            if (this.getRecipe() != null) {
                this.level.setBlockAndUpdate(this.getBlockPos(),
                        this.getBlockState().setValue(TCforge.LIT, false));
    ...
    private ITextComponent getDefaultName() {
        return new TranslationTextComponent("container." + techcraft.MOD_ID + ".tc_forge");
    }
    ...
    @Nullable
    public TCforgeRecipe getRecipe() {
        Set<IRecipe<?>> recipes = findRecipesByType(RecipeSerializerInit.TC_FORGE_TYPE, this.level);
        for (IRecipe<?> iRecipe : recipes) {
            TCforgeRecipe recipe = (TCforgeRecipe) iRecipe;
